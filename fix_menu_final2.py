# -*- coding: utf-8 -*-
# Strategy: restore from build_all.py, then replace ONLY specific methods

import subprocess, re

# Step 1: Get clean base
subprocess.run(['python', r'f:\YangshipinWrapper4TV-main\build_all.py'], cwd=r'f:\YangshipinWrapper4TV-main', capture_output=True)
...
# Helper: replace from start_tag (inclusive) to end_tag (exclusive)
def replace_between(content, start_tag, end_tag, new_content):
    start = content.index(start_tag)
    end = content.index(end_tag, start)
    return content[:start] + new_content + content[end:]

# 1. Replace toggleMenu... selectMenuItemAt
old_menu1_start = "    private void toggleMenu()"
old_menu1_end = "    private void appendNumber(int digit)"
new_menu1 = """    private void toggleMenu() {
...
c = replace_between(c, old_menu1_start, old_menu1_end, new_menu1)
print("Step 1: menu methods done")

# 2. Replace commitNumberInput... pointToMenuPosition
old_menu2_start = "    private void commitNumberInput()"
old_menu2_end = "    private Rect getListRectInRoot()"
new_menu2 = """    private void commitNumberInput() {
...
c = replace_between(c, old_menu2_start, old_menu2_end, new_menu2)
print("Step 2: key/touch handling done")

# 3. Replace ChannelAdapter
old_adapter_start = "    private final class ChannelAdapter extends BaseAdapter"
old_adapter_end = "    private final class GestureTraceView"
new_adapter = """    private final class ChannelAdapter extends BaseAdapter {
...
c = replace_between(c, old_adapter_start, old_adapter_end, new_adapter)
print("Step 3: ChannelAdapter done")

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(c)

print(f"\nAll 3 steps done. Final size: {len(c)}")