...
$order_res = Db::table('goods_order','as O')->field('O.*,G.appid,G.type,G.amount,A.pay_wx_eid,A.pay_wx_ekey')->JOIN("goods","as G",'O.gid=G.id')->JOIN("app",'as A','G.appid=A.id')->where(['O.order'=>$order])->find();//false
if(!$order_res)die('fail');

//商户ID
$alipay_config['partner']= $order_res['pay_wx_eid'];
//商户KEY
$alipay_config['key']= $order_res['pay_wx_ekey'];
//↑↑↑↑↑↑↑↑↑↑请在这里配置您的基本信息↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
