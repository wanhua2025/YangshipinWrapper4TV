...
Hidden:false
Name:算法解析
Url:live_analysisadm
Version:1.0
*/
if(!isset($islogin))header("Location: /");//非法访问
if(Db::table('live_analysis')->exist()){//判断数据表是否存在
	$nums=Db::table('live_analysis')->count();//获取用户总数
    $page=isset($_GET['page']) ? intval($_GET['page']) : 1;
    $url="./?live_analysisadm&page=";
    $bnums=($page-1)*$ENUMS;
}else{
$sql = "CREATE TABLE `{$DP}live_analysis` (
  ...
  KEY `header` (`header`(333))
	...
	if($res){
		echo "<script>location.href='./?live_analysisadm';</script>";
	}
}
?>
                        ...
                        <table class="table table-centered table-striped dt-responsive nowrap w-100">
                            <thead>
                                <tr>
									...
									<th><center><span class="badge badge-light-lighten">关键字</span></center></th>
                            ...
                            <tbody>
								<?php
									$app = Db::table('live_analysis','as A')->field('A.id,A.name,A.keyword,A.url,A.state,A.header,A.Core');
									if($so){
                                ...
                                <tr>
									...
									<td>
										<center>
                                            <?php if($rows['Core'] == 0):?>
                                                ...
                                                <?php elseif($rows['Core']== 5): ?>
                                                    <span class="badge badge-secondary">VLC内核
                                    ...
                                    <td>
                                        <center><a href="./?live_analysisedit&id=<?php echo $rows['id']; ?>" class="action-icon"> <i class="mdi mdi-border-color"></i></a></center>
                                    </td>
                                </tr>
                                <?php } ?>
                            </tbody>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="pl-3 pr-3" method="post">
						...
						<div class="form-group">
							<label>关键字</label>
							<input class="form-control" type="text" id="add_keyword" placeholder="Y://" value="" required>
						</div>
	...
	<script>
		$('#add_submit').click(function() {
			...
			$.ajax({
				cache: false,
				type: "POST",//请求的方式
				url : "ajax.php?act=live_analysis_addanalysis",//请求的文件名
				data : {
				    name:add_name,
		...
		function delsubmit(){
			...
			$.ajax({
				cache: false,
				type: "POST",//请求的方式
				url : "ajax.php?act=live_analysis_delanalysis",//请求的文件名
				data : {id:id_array},
				dataType : 'json',
				success : function(data) {
					if(data.code == 200){
		...
		function edit_state(id,state) {
			...
			$.ajax({
				cache: false,
				type: "POST",//请求的方式
				url : "ajax.php?act=live_analysis_state",//请求的文件名
