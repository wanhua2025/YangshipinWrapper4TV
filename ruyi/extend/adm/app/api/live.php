...
*/
    ...
    if($act == 'add'){//添加视频跑马公告
    	...
    	$app_notices = Db::table('app_live_notice')->where('appid',$appid)->find();
    	if ($app_notices['appid']!="") json(201,'公告已存在');
    	
    	$app_res = Db::table('app')->where('id',$appid)->find();
    	if(!$app_res)json(201,'应用不存在');
    	
    	$add_res = Db::table('app_live_notice')->add(['adm'=>$adm,'content'=>$content,'appid'=>$appid,'time'=>time()]);
    	//die($add_res); 
    	if($add_res){
    		json(200,'添加成功');
    	}json(201,'添加失败');
    ...
    if($act == 'del'){//删除视频通知
    	...
    	$res = Db::table('app_live_notice')->where('id',$id)->del();//false
    	if($res){
    		json(200,'删除成功');
    	}json(201,'删除失败');
    }
