...
if(Db::table('app_voice_notice')->exist()){//判断数据表是否存在
	$nums=Db::table('app_voice_notice')->count();
	$page=isset($_GET['page']) ? intval($_GET['page']) : 1;
	$url="/?app_voice&page=";
	$bnums=($page-1)*$ENUMS;
}else{
	$sql = "CREATE TABLE IF NOT EXISTS `{$DP}app_voice_notice` (
	...
	if($res){
		echo "<script>location.href='./?app_voice';</script>";
	}
}
?>
   ...
   <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <textarea class="form-control form-control-light mb-2" placeholder="请输入要发布的内容" id="add_content" name="add_content" rows="10"></textarea>
					<div class="row mt-2">
						<div class="col-lg-8">
						...
						<div class="col-lg-4">
							<div class="text-lg-right">
								<div class="btn-group ml-2">
									<button type="button" class="btn btn-primary btn-sm" name="add_submit" id="add_submit">确认发布</button>
								</div>
							</div>
						</div><!-- end col-->
                    </div>
    ...
    <!-- end row -->
	<?php
		$res = Db::table('app_voice_notice','as N')->field('N.*,A.name as appname')->JOIN('app','as A','N.appid=A.id')->order('id desc')->limit($bnums,$ENUMS)->select();
		foreach ($res as $k => $v){$rows = $res[$k];
	?>
	<div class="row">
        <div class="col-12">
	...
	<script>
		$('#add_submit').click(function() {
			...
			$.ajax({
				cache: false,
				type: "POST",//请求的方式
				url : "ajax.php?act=app_voice_add",//请求的文件名
				data : {
					adm:add_name,
					content:add_content,
					appid:add_appid
				...
				success : function(data) {
					...
					document.getElementById('add_submit').innerHTML="确认发布";
					if(data.code == 200){
		...
		function del(id){
			...
			$.ajax({
				cache: false,
				type: "POST",//请求的方式
				url : "ajax.php?act=app_voice_del",//请求的文件名
				data : {id:id},
				dataType : 'json',
				success : function(data) {
					if(data.code == 200){
