...
if(Db::table('app_vod_notice')->exist()){//判断数据表是否存在
	$nums=Db::table('app_vod_notice')->count();
	$page=isset($_GET['page']) ? intval($_GET['page']) : 1;
	$url="/?app_vod&page=";
	$bnums=($page-1)*$ENUMS;
}else{
	$sql = "CREATE TABLE IF NOT EXISTS `{$DP}app_vod_notice` (
	...
	if($res){
		echo "<script>location.href='./?app_vod';</script>";
	}
}
?>
   ...
   <div class="row">
    ...
    <!-- end row -->
	<?php
		$res = Db::table('app_vod_notice','as N')->field('N.*,A.name as appname')->JOIN('app','as A','N.appid=A.id')->order('id desc')->limit($bnums,$ENUMS)->select();
		foreach ($res as $k => $v){$rows = $res[$k];
	?>
	<div class="row">
        <div class="col-12">
	...
	<script>
		$('#add_submit').click(function() {
			...
			$.ajax({
				cache: false,
				type: "POST",//请求的方式
				url : "ajax.php?act=app_vod_add",//请求的文件名
				data : {
					adm:add_name,
					content:add_content,
					appid:add_appid
		...
		function del(id){
			...
			$.ajax({
				cache: false,
				type: "POST",//请求的方式
				url : "ajax.php?act=app_vod_del",//请求的文件名
				data : {id:id},
				dataType : 'json',
				success : function(data) {
					if(data.code == 200){
