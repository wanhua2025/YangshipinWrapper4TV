...
if(Db::table('app_text_notice')->exist()){//判断数据表是否存在
	$nums=Db::table('app_text_notice')->count();
	$page=isset($_GET['page']) ? intval($_GET['page']) : 1;
	$url="./?app_texts&page=";
	$bnums=($page-1)*$ENUMS;
}else{
	$sql = "CREATE TABLE IF NOT EXISTS `{$DP}app_text_notice` (
	...
	if($res){
		echo "<script>location.href='./?app_texts';</script>";
	}
}
?>
   ...
   <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <textarea class="form-control form-control-light mb-2" placeholder="二维码地址 留空则使用默认二维码" id="erweimaurl" name="erweimaurl" rows="1"></textarea>
                    
                    <textarea class="form-control form-control-light mb-2" placeholder="请输入要发布的内容,不得高于12个汉字" id="add_content" name="add_content" rows="3"></textarea>
					<div class="row mt-2">
						<div class="col-lg-8">
    ...
    <!-- end row -->
	<?php
		$res = Db::table('app_text_notice','as N')->field('N.*,A.name as appname')->JOIN('app','as A','N.appid=A.id')->order('id desc')->limit($bnums,$ENUMS)->select();
		foreach ($res as $k => $v){$rows = $res[$k];
	?>
	<div class="row">
        <div class="col-12">
	...
	<script>
		$('#add_submit').click(function() {
			let t = window.jQuery;
			var erweimaurl = $("textarea[name='erweimaurl']").val();
			...
			$.ajax({
				cache: false,
				type: "POST",//请求的方式
				url : "ajax.php?act=app_texts_add",//请求的文件名
				data : {
					adm:add_name,
					content:add_content,
					appid:add_appid,
		...
		function del(id){
			...
			$.ajax({
				cache: false,
				type: "POST",//请求的方式
				url : "ajax.php?act=app_texts_del",//请求的文件名
				data : {id:id},
				dataType : 'json',
				success : function(data) {
					if(data.code == 200){
