...
did = 'tv_final_pay_001'
acc = 'auto_' + did

print('=== REGISTER (param=user) ===')
r, raw = call_api('user_reg', {'user': acc, 'password': 'tvpass01', 'markcode': did, 't': int(time.time())})
print('reg code:', r.get('code'), 'dec:', rc4_decrypt(str(r.get('msg','')))[:200])

print('\n=== LOGIN (param=account) ===')
r, raw = call_api('user_logon', {'account': acc, 'password': 'tvpass01', 'markcode': did, 't': int(time.time())})
...
try:
    ...
    print('parse err:', e)
    if isinstance(r.get('msg'), str) and len(r['msg']) > 40:
        token = r['msg']
        print('using raw token: ' + token[:40])
...
print('\n=== GOODS ===')
r, raw = call_api('goods', {'t': int(time.time()), 'token': token})
dec = rc4_decrypt(str(r.get('msg','')))
print('goods dec:', dec[:500])

print('\n=== PAY (order+token+account+way+gid) ===')
order_no = time.strftime('%Y%m%d%H%M%S') + str(random.randint(10000, 99999))
print('order_no:', order_no)
r, raw = call_api('pay', {'t': int(time.time()), 'token': token, 'order': order_no, 'account': acc, 'way': 'wx', 'gid': 1, 'ua': 2})
print('pay code:', r.get('code'))
print('raw body:', raw[:800])
raw_msg = str(r.get('msg',''))
print('raw_msg hex:', raw_msg[:100])
dec = rc4_decrypt(raw_msg)
print('decrypted:', dec[:1000])

print('\n=== PAY_RES ===')
for i in range(3):
    r, raw = call_api('pay_res', {'t': int(time.time()), 'token': token, 'oid': order_no})
    dec = rc4_decrypt(str(r.get('msg','')))
