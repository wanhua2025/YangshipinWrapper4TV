...
# Find the block from toggleMenu() to just before GestureTraceView
start_tag = "    private void toggleMenu()"
end_tag = "    private final class GestureTraceView"

start_idx = c.index(start_tag)
end_idx = c.index(end_tag)

print(f"Block to replace: {start_idx} -> {end_idx} (len={end_idx-start_idx})")

new_block = '''    private void toggleMenu() {
    ...
    private int pointToMenuPosition(float x, float y) {
        ...
        if (rect == None || !rect.contains((int) x, (int) y)) return -1;
...
print(f"Replaced block. New file size: {len(new_content)}")